...
**核心目标**：
- 性能极限：追求最高通关率和得分
- 推理扩展性：专为 RTX 3080（20GB VRAM）优化推理部署
...
**预期性能**：
- 模型加载: ~1GB (FP16) - 500MB (INT8)
- 单帧推理延迟: 40-80ms (FP16 full 16步)
- 游戏帧率: 15-20 FPS（原生）
- Batch 吞吐: 16 并行请求，延迟 <100ms (p95)

**RTX 3080 (20GB) 约束下的最优配置**：
```
FP16 全精度 + 16步采样 + CFG=1.5 + FlashAttention + batch=16
= ~60ms 延迟 + ~4GB VRAM（还有大量余量）
```
可进一步扩展: batch=32, KV cache 优化, 甚至尝试更大的模型变体。
